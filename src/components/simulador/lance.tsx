...
export function LanceSimulador({
  ...
  const [percentualLanceEmbutido, setPercentualLanceEmbutido] = useState("30");
  const percentualLanceEmbutidoNumerico =
    Number(percentualLanceEmbutido.replace(/\D/g, "")) || 0;
  const lanceEmbutido =
    creditoInicial * (percentualLanceEmbutidoNumerico / 100);
  ...
  // Crédito líquido após considerar o lance embutido configurado
  const creditoLiquido = creditoInicial - lanceEmbutido;
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  min={0}
                  max={100}
                  value={percentualLanceEmbutido}
                  onChange={(e) => {
                    const rawValue = e.target.value;

                    if (rawValue === "") {
                      setPercentualLanceEmbutido("");
                      return;
                    }
                    ...
                    if (nextValue < 0) {
                      setPercentualLanceEmbutido("0");
                      return;
                    }

                    if (nextValue > 100) {
                      setPercentualLanceEmbutido("100");
                      return;
                    }

                    setPercentualLanceEmbutido(rawValue);
                  }}
                  className="w-25 h-9 border-purple-200/60 bg-white/70 text-[15px] font-semibold text-purple-700 dark:border-purple-800/60 dark:bg-purple-950/20 dark:text-purple-300"
                />
              ...
              <div className="text-right">
                ...
                <p className="text-[9px] text-purple-600/70 dark:text-purple-400/70">
                  {percentualLanceEmbutido || "0"}% do crédito inicial
                </p>
              </div>
            </div>
          </div>
